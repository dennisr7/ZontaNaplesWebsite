...
export const sendEventReminder = async (event, attendeeEmail) => {
    const mailOptions = {
        from: process.env.EMAIL_FROM,
        to: attendeeEmail,
        subject: `Event Reminder: ${event.title} - Coming Up Soon!`,
...
export const sendMembershipRenewalReminder = async (member, paymentUrl) => {
    ...
    try {
        await transporter.sendMail(mailOptions);
        console.log(`Membership renewal reminder sent to ${member.email}`);
    } catch (error) {
        console.error('Error sending membership renewal reminder:', error);
